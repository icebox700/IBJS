...
(function(Browser) {
    ...
    Browser.check_browser = function(browser) {
        if (browser.version <= Browser.supported_browsers[browser.name]) {
            Browser.notify(browser);
            return false;
        } else
            return true;
    };
    Browser.notify = function(browser) {
        console.error("\u00A1!The current browser is not supported on this site!\u00A1 You may encounter problems. - " + browser.name + " | " + browser.version + " -  Supported Browsers: " + Browser.supported_browsers.toString());
    };
})(Browser || (Browser = {}));
/***************************************
...
// Browser Check
var current_browser = Browser.get_browser();
